...
通过 HTML API 直接获取页面数据，使用 ?hastopwindow=1 参数
"""
...
from .api_crawler import APIPageCrawler
from .storage import DataStorage

logger = logging.getLogger(__name__)


class APICrawlerScheduler:
    """API 爬虫调度器"""
    
    def __init__(self, config: dict):
        """初始化调度器"""
        self.config = config
        self.crawler = APIPageCrawler(config)
    ...
    def start(self):
        """启动爬虫"""
        logger.info("=" * 60)
        logger.info("开始爬取 vivo 手表文档 (API 模式)")
        logger.info("=" * 60)
        ...
        try:
            ...
            paths = self.crawler.discover_pages_from_sitemap()
    ...
    def _crawl_page(self, path: str) -> bool:
        ...
        try:
            ...
            # 构造完整 URL
            full_url = f"https://developers-watch.vivo.com.cn{path}"
